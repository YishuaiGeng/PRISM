...
### 🟢 2026-07-10（下午）：双种子复现 + GPT-4o 对照 + 新立意论文草稿

- **seed-123 复现（8 run 完成，basesparc_gB 因 mmm 503 崩溃已重跑中）**：
  baseline 9.5%（SBW 28）｜nopar 10.5%（SBW 26）｜noparsparc 10.5%
  （SBW **8**，弃答 17）。**SBW 消减稳定复现（69%），但准确率增益未复现**
  （vs nopar +0.0pp）；合并 n=400：vs baseline **+1.75pp p=0.032（显著）**、
  vs nopar +1.5pp p=0.065（边缘）。稳健主张改锚定可靠性：作答条件错误率
  55%→**24%**（覆盖=产出 SAT 答案口径）
- **GPT-4o 对照行完成**：baseline 25.5%（SBW 4）｜noparsparc 24.0%
  （SBW 6，弃答 1），−1.5pp 双侧 p≈0.10 不显著 → 能力梯度叙事成立
  （强模型 SBW 天然 2%，SPARC 为近零成本保险）
- **新立意论文草稿**：`docs/paper_draft/sparc_paper_zh.tex`（v0.2，15 页
  编译通过，标准五段式框架，双种子真实数字已入表；bib 4 条【待核实】）
- **基础设施**：mmm 503 过载重试加固（`llm_api._request_with_retry`，
  过载类扩到 10 次指数退避）；消融三开关已实现+测试
  （`--sparc-max-completions 0`=仅门控 / `--sparc-blind-completion` /
  `--sparc-no-invariant`，436 测试过）
- **seed-7 全矩阵 8 run 已发**（3 种子锁表的第三种子）
- ⚠️ 方法-代码对齐：论文 §4.4"覆盖账本反查线索并排序候选"在 zebra 实现
  中未做（prompt 直接给全文+差分摘要）——写作时软化措辞或补实现；
  "−覆盖账本"消融因此不可做，消融表按三开关设计

### 🟢 2026-07-10：修正后矩阵 seed-42 —— RQ4 通过（首个干净的显著正结果）

- 可计分 200 题（440 题官方留白）×6 系统，全部干净度量：
  baseline 10.5%（SBW **25**）｜nopar 10.0%（SBW 25）｜nomem 11.0%｜full 9.0%
  ｜**baseline+SPARC 11.5%（SBW 5，弃答 16）**
  ｜**nopar+SPARC 13.0%（SBW 7，弃答 13）**
- 配对 bootstrap：noparsparc vs nopar **+3.0pp p=0.015**；
  vs baseline **+2.5pp p=0.034**；SBW 消减 72–80%；成本仅 +0.25 调用/题
- **修正后的新发现：SBW 是基线流水线的地方病**（12.5% 的可计分结果是
  SAT但错），不是记忆特有的放大产物（v1 的"剂量-响应"叙事被修正数据
  否定，论文 §2 需改写为"endemic SBW + 门控修复"）；SAT对 20→26 说明
  差分补全在**恢复正确答案**而不只是弃答
- 待办：3 种子锁表；GPT-4o 对照行；RQ2/RQ3 分解（检出查全 80%=20/25，
  5 例逃逸为误约束型）；自生成评测集补统计功效（可计分仅 200）；
  AR-LSAT 良定性门控实例化

### 🟠 2026-07-10：三重评测 bug 曝光——zebra 旧定量作废（已修复）

- RQ4 首跑的异常弃答率引出连环排查，发现三个叠加 bug：
  ① 官方测试集 440/640 题答案留白（'___'）→ 评分回退"SAT 即 solved"（假阳性）；
  ② 有真值题的字符串比较因命名规范差异永不匹配（真实正确答案被判错，
  实测同一题 `BookGenre_fantasy=house2` vs `bookGenre_Fantasy=2` 语义全同）；
  ③ 模型校验用真值键做 schema 对齐 → 所有有真值题的合法 SAT 被判
  MISALIGNED_MODEL + 触发 594/640 的无谓重翻译（污染求解行为本身）
- 修复：`model_validation._normalise_schema_key`、`zebralogic.answers_match/
  is_scorable`（留白题不计分而非算对）、`rescore_zebra_results.py`；468 测试过
- **SPARC 三机制已实现**（`--sparc`：π-gate + 差分补全带进展保证 +
  冲突修复带证据保护），10 个单测含文档 §2.4.5 例子的逐字复现
- 修正后全矩阵重跑中：6 系统（4 原系统 + baseline/nopar 的 SPARC 臂）×
  640 × seed 42（`results/zebra_v2_s42/`）。**可计分子集 n=200**
  （440 题官方无答案），统计功效受限——后续考虑补自生成评测集或找全答案源

### 🔴 ~~最终判决（2026-07-09）：zebra 主表 seed-42 — 机制反向~~（数字作废，见上）
